...
void dfb_surface_unlock( CoreSurface *surface, int front )
{
     ...
     D_HEAVYDEBUG( "Core/Surface: dfb_surface_unlock( %p, %s )\n", surface, front ? "front" : "back" );
...
DFBResult dfb_surface_dump( CoreSurface *surface,
                            ...
                            const char  *prefix )
{
    return dfb_surface_dump_buffer(surface, directory, prefix, true);
}
                            ...
                            bool         front )
{
     ...
     ret = dfb_surface_soft_lock( surface, DSLF_READ, &data, &pitch, front );
     if (ret) {
          if (palette)
               dfb_palette_unref( palette );
          return ret;
     ...
     switch (policy) {
          ...
          case CSP_VIDEOONLY: {
               ...
               if (ret == DFB_NOVIDEOMEMORY) {
                   ...
                   D_ALLOCATION( "Core/Surface: allocated buffer 0x%08X from %s %d\n", (unsigned int)buffer->video.offset, file, line);
#endif
               }
                                 ...
                                 SurfaceBuffer *buffer )
{
     ...
     if (buffer->video.health && !(buffer->flags & SBF_FOREIGN_VIDEO)) {
          D_ALLOCATION( "Core/Surface: deallocating buffer 0x%08X from %s %d\n", (unsigned int)buffer->video.offset, file, line);
          dfb_surfacemanager_deallocate( surface->manager, buffer );
     }
