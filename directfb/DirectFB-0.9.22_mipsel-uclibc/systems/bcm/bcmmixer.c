...
DFBResult BCMMixerSetDisplayDimensions(BCMMixerHandle mixer,
    ...
    source2destination_init(output_layer);
...
BCMMixerApplyClearrect(BCMMixerHandle mixer,
    ...
    for (i = 0; i < mixer->subpic_cr.number; i++)
    {
        ...
        src2display_rect_transform(ihd_layer, &mixer->subpic_cr.rects[i].rect, &display_rect);
                    ...
                    if (first_surface) {
                        if (current_layer->bcmcore_type == BCMCORELAYER_FAA) {
                            if ((ret = BCMMixerBlit(
                                        ...
                                        &output_layer->dirty_rectangles[output_layer->back_surface_index],
                                        NULL,
                                        BGFX_RULE_PORTERDUFF_SRC)) != DFB_OK) {
                                goto quit;
                            }
                            ...
                            if ((ret = BCMMixerBlit(
                                        ...
                                        NULL,
                                        current_layer->premultiplied ? BGFX_RULE_PORTERDUFF_SRCOVER : BGFX_RULE_SRCALPHA_INVSRCALPHA)) != DFB_OK) {
                                goto quit;
                            }
                            }
                        }
                            ...
                            if ((mixer->subpic_cr.number) && (bdvd_dfb_ext_hddvd_subpicture_layer == first_surface_layer->dfb_ext_type))
                            {
                                /*
                                 * blit sub picture layer first to output layer
                                 */
                                if ((ret = BCMMixerBlit(
                                            mixer->mixer_compositor,
                                            ((bdvd_graphics_surface_h*)output_layer->surface_buffers[output_layer->back_surface_index]->video.ctx)[0],
                                ...
                                /*
                                 * clearrect output layer containing subpicture.
                                 */
                                BCMMixerApplyClearrect(mixer, first_surface_layer, output_layer);

                                /*
                                 * blit the second layer in top of it.
                                 */
                                if ((ret = BCMMixerBlit(
                                            mixer->mixer_compositor,
                                            ((bdvd_graphics_surface_h*)output_layer->surface_buffers[output_layer->back_surface_index]->video.ctx)[0],
                                            ...
                                            &output_layer->dirty_rectangles[output_layer->back_surface_index],
                                            NULL,
                                            BGFX_RULE_PORTERDUFF_SRCOVER)) != DFB_OK) {
                                    goto quit;
                                }
                            }
                            else
...
/* Disable Memory Optimization: side effect of BD STD100 title 19 (TBF) */
#if 0
                /* The logic will need to be replaced based on the number of layer registered to the mixer */
                if ((dfb_bcmcore_context->graphics_constraints.nb_gfxWindows[0] != 1) &&
                    ((bdvd_dfb_ext_hddvd_subpicture_layer == current_layer->dfb_ext_type) ||
                    (bdvd_dfb_ext_bd_hdmv_presentation_layer == current_layer->dfb_ext_type)))
                {
                    bdvd_graphics_blending_rule_e blending_rule = current_layer->premultiplied ? BGFX_RULE_PORTERDUFF_SRC : BGFX_RULE_SRCALPHA_ZERO;
                    DFBRectangle src_rect;

                    destination2source_transform(current_layer, &output_layer->dirty_rectangles[output_layer->back_surface_index], &src_rect);

                    if ((ret = BCMMixerBlit(
                                mixer->mixer_compositor,
                                ((bdvd_graphics_surface_h*)output_layer->surface_buffers[output_layer->back_surface_index]->video.ctx)[0],
                                ((bdvd_graphics_surface_h*)current_layer->surface_buffers[current_layer->back_surface_index]->video.ctx)[0],
                                NULL,
                                &output_layer->dirty_rectangles[output_layer->back_surface_index],
                                &src_rect,
                                NULL,
                                blending_rule)) != DFB_OK) {
                        goto quit;
                    }

                    D_DEBUG("blit src x=%d y=%d w=%d h=%d out x=%d y=%d w=%d h=%d\n",
                        src_rect.x,
                        src_rect.y,
                        src_rect.w,
                        src_rect.h,
        ...
        if (dfb_bcmcore_context->secondary_graphics_window.layer_assigned) {
            ...
            source2destination_transform(dfb_bcmcore_context->secondary_graphics_window.layer_assigned, &dfb_source_rect, &dfb_dest_rect);

            if ((ret = BCMMixerBlit(
                          mixer->mixer_compositor,
                          ((bdvd_graphics_surface_h*)((BCMCoreLayerHandle)(dfb_bcmcore_context->secondary_graphics_window.layer_assigned))->surface_buffers[((BCMCoreLayerHandle)(dfb_bcmcore_context->secondary_graphics_window.layer_assigned))->front_surface_index]->video.ctx)[0],
                          ((bdvd_graphics_surface_h*)output_layer->surface_buffers[output_layer->back_surface_index]->video.ctx)[0],
                          NULL,
                          &dfb_dest_rect,
                          &dfb_source_rect,
...
static DFBResult BCMMixerSyncFlip(BCMMixerHandle mixer, bool wait_vsync) {
    ...
    if (flipping) {
        ...
        if (dump_layer_id_string) {
            if (ret = dfb_surface_dump_buffer( output_layer->dfb_core_surface, ".", "framebuffer", false) != DFB_OK) {
                goto quit;
            }
        }
#endif
...
/* add a wait parameter */
    ...
    D_DEBUG("%s/BCMMixerLayerFlip called with:\n"
            "layer_id: %u\n",
            BCMMixerModuleName,
            layer_id );
    ...
    switch (layer->bcmcore_type) {
        case BCMCORELAYER_MIXER_FLIPBLIT:
            if (layer->nb_surfaces < 2) {
                D_DEBUG( "%s/BCMMixerLayerFlip: layer must be at least double_buffered, skipping the blit\n",
                         BCMMixerModuleName);
                goto quit;
            }
            ...
            if (update) {
                ...
                source2destination_transform(layer, &dfb_source_rect, &dfb_dest_rect);

                D_DEBUG("%s/BCMMixerLayerFlip: (from update) source rect %d %d %d %d\n",
                        BCMMixerModuleName,
                        dfb_source_rect.x,
                        dfb_source_rect.y,
                        dfb_source_rect.w,
                ...
                D_DEBUG("%s/BCMMixerLayerFlip: (update region) dest rect %d %d %d %d\n",
                        BCMMixerModuleName,
                        dfb_dest_rect.x,
                        dfb_dest_rect.y,
                        dfb_dest_rect.w,
                ...
                D_DEBUG("%s/BCMMixerLayerFlip: (update region) dirty reg %d %d %d %d\n",
                        BCMMixerModuleName,
                ...
                D_DEBUG("%s/BCMMixerLayerFlip: (update region) dirty rec %d %d %d\n",
                        BCMMixerModuleName,
                        temp_rectangle.x,
                        temp_rectangle.y,
                        temp_rectangle.w,
                ...
                D_DEBUG("%s/BCMMixerLayerFlip: (from layer settings) source rect %d %d %d %d\n",
                        BCMMixerModuleName,
                        layer->back_rect.x,
                        layer->back_rect.y,
                        layer->back_rect.w,
                ...
                D_DEBUG("%s/BCMMixerLayerFlip: (from layer settings) dest rect %d %d %d %d\n",
                        BCMMixerModuleName,
                        layer->front_rect.x,
                        layer->front_rect.y,
                        layer->front_rect.w,
                ...
                D_DEBUG("%s/BCMMixerLayerFlip: (update layer) rect %d %d %d %d\n",
                        BCMMixerModuleName,
                        layer->dirty_region.x1,
                        layer->dirty_region.y1,
                        layer->dirty_region.x2,
            ...
            /* The logic will need to be replaced based on the number of layer registered to the mixer */
            if ((dfb_bcmcore_context->graphics_constraints.nb_gfxWindows[0] != 1) &&
                 ((bdvd_dfb_ext_hddvd_subpicture_layer == layer->dfb_ext_type) ||
                 (bdvd_dfb_ext_bd_hdmv_presentation_layer == layer->dfb_ext_type)))
            {
/* Disable Memory Optimization: side effect of BD STD100 title 19 (TBF) */
#if 0
                skipcopy = true;
            ...
            if (!skipcopy)
            {
                if (layer->premultiplied) {
                    ...
                    if ((ret = BCMMixerBlit(
                                  ...
                                  NULL,
                                  update ? &dfb_dest_rect : &layer->front_rect,
                                  update ? &dfb_source_rect : &layer->back_rect,
                                  NULL,
                                  BGFX_RULE_PORTERDUFF_SRC)) != DFB_OK) {
                        goto quit;
                    }
                    ...
                    if ((ret = BCMMixerBlit(
                                  ...
                                  update ? &dfb_source_rect : &layer->back_rect,
                                  NULL,
                                  BGFX_RULE_SRCALPHA_ZERO)) != DFB_OK) {
                        goto quit;
                    }
                }
            }
        ...
        case BCMCORELAYER_BACKGROUND:
        {
            ...
            if (layer->level > 0)
            {
                ...
                bdvd_decode_window_get(bdvd_decode_get_window(B_ID(1)), &window_settings);
