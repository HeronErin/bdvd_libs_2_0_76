 ...
 */
static DFBResult
create_graphics_framebuffers(BCMMixerHandle mixer, BCMCoreLayerHandle layer, CoreSurface * surface )
{
...
uint8_t BCMCoreFrameFrequencyFactor()
{
    uint8_t retValue = 1;
    bdvd_video_format_e video_format = display_get_video_format(dfb_bcmcore_context->primary_display_handle);

    if ((video_format == bdvd_video_format_480p) ||
        (video_format == bdvd_video_format_576p) ||
        (video_format == bdvd_video_format_720p) ||
        (video_format == bdvd_video_format_720p_50hz))
    {
        /*
           pr 8298: for the following video frequency the frame vsync come twice
           as fast. However graphics still need to be rendered at the same rate.
        */
        retValue = 2;
    }
...
static DFBResult BCMCoreFlipRegion    ( CoreLayer                  *layer,
    ...
    if (dump_layer_id_string) {
        ...
        if (mixed_layer_id == dump_layer_id) {
            if (ret = dfb_surface_dump_buffer( surface, ".", dump_layer_id_string, false) != DFB_OK) {
                goto quit;
            }
        }
    }
                        ...
                        CoreSurface            **ret_surface )
{
    ...
    switch (bcm_layer->bcmcore_type) {
        case BCMCORELAYER_OUTPUT:
            ...
            if (*ret_surface == NULL) {
                ...
                if ( (ret = create_graphics_framebuffers(mixer, bcm_layer, &bcm_layer->frontsurface_clone)) != DFB_OK) {
                    goto quit;
                }
            }
    ...
    source2destination_init(bcm_layer);
                ...
                if ((ret = BCMMixerLayerAcquire(layer_ctx->mixer, layer_ctx->mixed_layer_id, &bcm_layer)) != DFB_OK) {
                    ...
                    if (dfb_bcmcore_context->secondary_graphics_window.layer_assigned) {
                        ...
                        if (!graphics_settings.enabled || (dfb_bcmcore_context->video_format != video_format)) {
                            ...
                            source2destination_init(dfb_bcmcore_context->secondary_graphics_window.layer_assigned);
                        }
                    }
...

        {
            ...
            if (force_alphapremult_string) {
                ...
                if (layer_ctx->mixed_layer_id == force_alphapremult_id) {
                    D_INFO("%s/BCMCoreSetRegion: forcing layer %u surface_caps DSCAPS_PREMULTIPLIED to be set\n",
                           BCMCoreModuleName,
