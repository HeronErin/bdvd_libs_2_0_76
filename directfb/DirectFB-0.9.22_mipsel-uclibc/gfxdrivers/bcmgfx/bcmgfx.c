...
static void BCMGfxEngineSync( void *drv, void *dev )
{
    ...
    {
        BCMCoreContext * dfb_bcmcore_context = (BCMCoreContext *)dfb_system_data();
        if ((err = bdvd_graphics_compositor_sync(dfb_bcmcore_context->mixer_compositor)) != b_ok) {
    ...
    if (bdvd_dtk_get_handle())
    {
        ...
        while (bdvd_dtk_is_idle(dfb_bcmcore_context->driver_fgx) == false)
        {
            /* add something */
        }
    }
...
static inline bool BCMGfxUnifiedBlit(
    ...
    {
        BCMCoreContext * dfb_bcmcore_context = (BCMCoreContext *)dfb_system_data();
        compositor = dfb_bcmcore_context->mixer_compositor;
    }
        ...
        /* This copies the source and sets the destinations alpha channel
           to opaque. */
            ...
            if ((err = bdvd_graphics_surface_blit(
                            ...
                            drawing ? false : bcmgfxdev->fade_effect_src_prem)) != b_ok) {
...
#ifdef ENABLE_MORE_DEBUG
                D_DEBUG( "%s/BCMGfxFillRectangle: color blending rule is BGFX_RULE_PORTERDUFF_SRC_CONSTANTSRC\n",
                        BCMGfxModuleName );
#endif
                if ((err = bdvd_graphics_surface_fill(
                              compositor,
