                                 ...
                                 const DFBRectangle     *destination_rect )
{
    ...
    if (converter_singleton->enabled) {
		...
		{
        if (!destination_differs) destination_differs = (dst_surface->format != image_surface_desc.pixelformat);
		}
 ...
 /*PR12701*/ if (getenv("sid_use_intermediate_buffer"))
 			 {
            	if (!destination_differs) destination_differs = (desired_pitch != pitch);
        	 }
        }
        else {
