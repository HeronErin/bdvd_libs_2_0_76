                                             ...
                                             SurfaceBuffer  *buffer )
{
    ...
    //take a chunk from the list
     ...
     if(c==NULL)
     {
     	...
     	//cant do?
     	if(result!=DFB_OK) {
     		D_ALLOCATION("Core/SurfaceMgrBuddy -> allocating %d requires locking %d but...\n",length,(1<<order)*manager->page_size);
...

     {
     	...
     	D_ALLOCATION(
     		"Core/SurfaceMgrBuddy -> reserved %d at 0x%08X for %d (%d wasted) %d avail.\n",size,offset,length,size-length,manager->available);
     }
